...
class WorkerDrone(Worker):
    ...
    def __init__(self, beam, beam_connector):
        ...
        self.propellers = [p for p in self.model.find_all_matches("**/propeller_*")]
        base.task_mgr.add(self.rotate_propellers, "rotate_propellers")

    def rotate_propellers(self, task):

        for propeller in self.propellers:
            h = propeller.get_h() - 1000. * globalClock.get_dt()
            propeller.set_h(h)
...
class DroneCompartment:
    ...
    def __init__(self):
        ...
        self.model = base.loader.load_model("models/worker_drone_compartment.gltf")
    ...
    def open_iris(self, task):
        ...
        if self.blade_angle <= 0.:
            ...
            base.task_mgr.add(self.close_iris, "close_iris", delay=2.)
...
class StarshipPlatform:

    def __init__(self, job_starter):

        self.model = base.loader.load_model("models/starship_platform.gltf")
        self.model.reparent_to(base.render)
        self.model.set_color(.2, .2, .2, 1.)
        self.model.set_material_off(1)
        self.model.set_texture_off(1)
        self.stairs = self.model.find_all_matches("**/platform_stair_step*")
        self.lights = self.model.find_all_matches("**/forcefield_generator_light*")
        ...
        '''
        self.forcefield = self.model.find("**/forcefield").children[0]
        self.forcefield.node().set_geom_state(0, RenderState.make_empty())
        self.forcefield.set_transparency(TransparencyAttrib.M_alpha)
        self.forcefield.set_light_off()
#        self.forcefield.set_two_sided(True)
        self.forcefield.set_color(.2, .2, 1., .2)
        attrib = CBA.make(CBA.M_none, CBA.O_incoming_color, CBA.O_incoming_color,
            CBA.M_add, CBA.O_incoming_alpha, CBA.O_one, (.5, .5, 1., 1.))
        self.forcefield.set_attrib(attrib)
        self.forcefield.set_bin("unsorted", -10)
        '''
        ...
        self.sliding_panel.set_color(.2, .2, .2, 1.)
        self.elevator_platform = self.model.find("**/elevator_platform")
        self.elevator_platform.set_color(.2, .2, .2, 1.)
        self.elevator_platform_start_z = self.elevator_platform.get_z()

        for i in range(14):
            elevator = Elevator(self.elevator_platform, -65. + i * 10.)

        self.job_starter = job_starter
        base.task_mgr.add(self.lower_panel, "lower_panel")
    ...
    def lower_panel(self, task):
        ...
        base.task_mgr.add(lambda task: self.slide_panel(task, -1.), "slide_panel")
    ...
    def slide_panel(self, task, direction):
        ...
        if direction < 0.:
            base.task_mgr.add(self.raise_elevator_platform, "raise_elevator_platform")
        else:
            base.task_mgr.add(self.raise_panel, "raise_panel")
    ...
    def deactivate_forcefield(self):
        ...
        base.task_mgr.add(self.lower_elevator_platform, "lower_elevator_platform")
    ...
    def raise_panel(self, task):
        ...
        base.task_mgr.add(self.raise_stairs, "raise_stairs")
...
#            elevator = Elevator(-90. + i * 10.)
        ...
        self.compartment = compartment = DroneCompartment()
        compartment.model.set_pos(0., 0., 55.)
        base.task_mgr.add(compartment.handle_next_request, "handle_compartment_requests")
        ...
        self.starship_platform = StarshipPlatform(self.start_jobs)

        base.task_mgr.add(self.check_workers_done, "check_workers_done")
    ...
    def check_workers_done(self, task):
        ...
        self.starship_platform.deactivate_forcefield()
